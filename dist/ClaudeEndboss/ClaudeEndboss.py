"""ClaudeEndboss - ein Pacman-Bot.

Diese Datei enthaelt die Klasse ``ClaudeEndboss`` und alles, was sie zum Spielen
...
---------
    ...
    from ClaudeEndboss import ClaudeEndboss

    pacmans = [[Pacman, "Pacman1"], [Pacman, "Pacman2"],
               [TRex, "Trex1"], [ClaudeEndboss, "ClaudeEndboss"]]
    walls = [[[5, 3], Direction.east, 8], [[5, 4], Direction.south, 3]]
    field = Field(15, pacmans, walls)

Fuer ``PacmanGame.py`` genuegt es, ``ClaudeEndboss`` zu importieren und in die
...
* Players act in a fixed order, ``for pacman in field.pacmans``, and our
  ``ClaudeEndboss`` is appended last - so when we act, every rival has already
  moved this round.
...
"""The brain behind ``ClaudeEndboss``.
...
TUNED_WEIGHTS = {
    "run_ahead": 0.5092476274583364,
    "run_discount": 0.7,
    "density": 0.2306303930609958,
    "hunt": 0.5707016575701158,
    "hunt_decay": 0.759902542978583,
    "exposure": 4.080579424569696,
    "facing_discipline": 0.0,
    "survival_bonus": 18.28829690521761,
    "discount": 0.999,
    "beam_width": 27,
    "depth": 14,
...
if __name__ == "__main__":
    ...
    def _partie(seed):
        ...
        pacmans = [[Pacman, "Pacman1"], [Pacman, "Pacman2"], [Pacman, "Pacman3"],
                   [_TRex, "Trex1"], [_TRex, "Trex2"], [ClaudeEndboss, "ClaudeEndboss"]]
        feld = _Field(15, pacmans, _walls)
        ich = next(p for p in feld.pacmans if p.name == "ClaudeEndboss")
        worst = 0.0
        zug = 0
    ...
    print("ClaudeEndboss Selbsttest - laeuft die Datei sauber durch?")
